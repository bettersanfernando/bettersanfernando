...
import { runStages, sourceArgument, syncAndVerify, validSource, VERIFY_STAGES } from './civic-workflow.mjs';

test('keeps every required civic verification stage in order', () => {
  assert.deepEqual(VERIFY_STAGES.map(([name]) => name), [
    'validate civic data', 'smoke civic data', 'check public data boundary',
    'test public data boundary', 'smoke search', 'smoke navigation',
    'build public app', 'check diff whitespace',
  ]);
});

test('runs civic verification stages in order', () => {
  const seen = [];
  assert.equal(runStages(['validate', 'build'], stage => (seen.push(stage), 0)), 0);
  assert.deepEqual(seen, ['validate', 'build']);
});

test('stops verification on the first failure', () => {
  const seen = [];
  assert.equal(runStages(['validate', 'build'], stage => (seen.push(stage), stage === 'validate' ? 9 : 0)), 9);
...
test('sync failure prevents verification', () => {
  const seen = [];
  assert.equal(syncAndVerify('C:/data', {
