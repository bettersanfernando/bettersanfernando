...
assert(
  projects.length === 306,
  `expected 306 projects, got ${projects.length}`
);
...
assert(
  ...
  'all 306 canonical project detail IDs must resolve'
);
...
assert(
  implementationReported.length === 66,
  'expected 66 IMPLEMENTATION_REPORTED projects'
);
assert(
...
assert(
  evidence.length === 572,
  `expected 572 evidence records, got ${evidence.length}`
);
assert(
