...
assert(
  projects.length === 303,
  `expected 303 projects, got ${projects.length}`
);
...
assert(
  ...
  'all 303 canonical project detail IDs must resolve'
);
...
assert(
  implementationReported.length === 41,
  'expected 41 IMPLEMENTATION_REPORTED projects'
);
